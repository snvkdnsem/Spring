...
public class Test {
	public static void main(String[] args) {
		 ...
		 * * one*(..) : 리턴자료형 메소드명(파라미터) : 필수 설정 항목(*리턴자료형, one*메소드명, (..) 파라미터)
		 * * : all : 리턴자료형이 무엇이든 상관이 없다. int, void, Object...등
...
//		pointcut.setExpression("execution(* one*(..))"); // 여기서 상관 있는건 메소드명인 one만 이다(*, (..) 아무거나 상관없다)
		
		// int형의 파라미터 2개를 받는 메소드에 어드바이스를 적용한다.
//		pointcut.setExpression("execution(* *(int, int))"); // (int, int): 현재 파라미터가 2개이다
		
		// 첫번째 파라미터로 int형의 값을 받는 메소드에 어드바이스를 적용한다.
//		pointcut.setExpression("execution(* *(int, ..))");
		
		// 파라미터를 2개이상 받는 메소드에 어드바이스를 적용한다.
//		pointcut.setExpression("execution(* *(*, *, ..))");
		
		// Second 클래스이고 메소드명에 "o"가 있는 메소드에 어드바이스를 적용한다.(알파벳 o라는것을 뽑아준다.)
		// Second."o" 클래스명만 붙이면 안된다.
